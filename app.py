...
# ══════════════════════════════════════════════
#  SOUND ENGINE  (Web Audio API injected once)
# ══════════════════════════════════════════════
SOUND_JS = """
<script>
(function(){
  if(window._axiomAudioReady) return;
  window._axiomAudioReady = true;

  const ctx = () => {
    if(!window._axiomCtx){
      window._axiomCtx = new (window.AudioContext || window.webkitAudioContext)();
    }
    return window._axiomCtx;
  };
  ...
  window.axiomSounds = {
    ...
    boot: function(){
      ...
      o.frequency.exponentialRampToValueAtTime(440, ac.currentTime+0.6);
      ...
      o.start(ac.currentTime); o.stop(ac.currentTime+0.7);
      playTone(880,'sine',0.25,0.06,0.5);
      playTone(1320,'sine',0.2,0.04,0.65);
    },

    scan: function(){
      // rapid blip sequence — analysis running
      [0,0.08,0.16,0.26,0.36,0.5,0.65,0.8].forEach((d,i)=>{
        const f = 300 + i*120;
        playTone(f,'square',0.06,0.05,d);
        playNoise(0.04,0.015,d+0.03);
      });
    },

    success: function(){
      // clean upward arpeggio
      [[523,0],[659,0.1],[784,0.2],[1047,0.32]].forEach(([f,d])=>
        playTone(f,'sine',0.3,0.07,d));
      playTone(1047,'triangle',0.6,0.04,0.35);
    },

    failure: function(){
      // harsh descending alarm
      playTone(880,'sawtooth',0.15,0.15,0);
      playTone(660,'sawtooth',0.15,0.15,0.12);
      playTone(440,'sawtooth',0.2, 0.18,0.26);
      playNoise(0.08,0.08,0);
      playNoise(0.08,0.08,0.12);
      // pulsed alarm
      [0.5,0.65,0.8,0.95].forEach(d=>playTone(523,'square',0.1,0.1,d));
    },

    click: function(){
      playTone(800,'square',0.04,0.04);
      playNoise(0.03,0.02,0.01);
    },
    ...
    warning: function(){
      ...
      playTone(440,'triangle',0.2,0.08,0.25);
      playTone(550,'sine',0.15,0.06,0.5);
    },
  ...
  window.playAxiom = function(name){
    try{ window.axiomSounds[name](); } catch(e){}
  };
})();
</script>
"""
...
# ══════════════════════════════════════════════
#  CSS — FULL AXIOM THEME + ANIMATIONS
# ══════════════════════════════════════════════
MASTER_CSS = r"""
<style>
...
/* ══════════════════════════════════════════════
   SIDEBAR TOGGLE  >>  fully visible + glowing
   ══════════════════════════════════════════════ */
...
[data-testid="stSidebarCollapseButton"] button{
  ...
  box-shadow:2px 0 16px rgba(65,244,224,0.22),inset -1px 0 0 rgba(65,244,224,0.1)!important;
  transition:all 0.2s ease!important;cursor:pointer!important;
}
[data-testid="stSidebarCollapseButton"] button:hover{
  background:rgba(65,244,224,0.12)!important;
  box-shadow:2px 0 30px rgba(65,244,224,0.45)!important;width:40px!important;
}
...
[data-testid="collapsedControl"] button{
  ...
  color:var(--cyan)!important;width:36px!important;height:56px!important;
  display:flex!important;align-items:center!important;justify-content:center!important;
  box-shadow:3px 0 20px rgba(65,244,224,0.28)!important;
  animation:sb-pulse 2.5s ease-in-out infinite!important;cursor:pointer!important;
}
[data-testid="collapsedControl"] button:hover{
  background:rgba(65,244,224,0.15)!important;width:44px!important;
  box-shadow:3px 0 36px rgba(65,244,224,0.55)!important;animation:none!important;
}
[data-testid="collapsedControl"] button svg{
  color:var(--cyan)!important;fill:var(--cyan)!important;stroke:var(--cyan)!important;
  filter:drop-shadow(0 0 6px rgba(65,244,224,0.95))!important;
  width:20px!important;height:20px!important;
}
@keyframes sb-pulse{
  0%,100%{box-shadow:3px 0 20px rgba(65,244,224,0.2);}
  50%{box-shadow:3px 0 36px rgba(65,244,224,0.55),0 0 0 4px rgba(65,244,224,0.07);}
}
[data-testid="collapsedControl"]::after{
  ...
  text-transform:uppercase;animation:sb-pulse-txt 2.5s ease-in-out infinite;
}
@keyframes sb-pulse-txt{0%,100%{opacity:0.4;}50%{opacity:0.9;}}
...
section[data-testid="stSidebar"]::before{
  ...
  animation:top-flow 4s linear infinite;
}
@keyframes top-flow{0%{background-position:0% 50%;}100%{background-position:200% 50%;}}
...
.stSlider>div>div>div>div{
  background:linear-gradient(90deg,var(--cyan),rgba(65,244,224,0.4))!important;border-radius:99px!important;
}
.stSelectbox>div>div{
  background:rgba(65,244,224,0.04)!important;border:1px solid rgba(65,244,224,0.22)!important;
  border-radius:4px!important;font-family:var(--fm)!important;font-size:13px!important;
}
...
@keyframes glow-pulse{
  0%,100%{box-shadow:0 0 0 0 rgba(65,244,224,0);}
  50%{box-shadow:0 0 24px rgba(65,244,224,0.18),0 0 48px rgba(65,244,224,0.08);}
}
...
@keyframes alarm-flash{
  0%,100%{background:rgba(255,71,87,0.06);}
  50%{background:rgba(255,71,87,0.16);}
}
...
.axiom-hero{
  ...
  overflow:hidden;border:1px solid var(--bdr);border-radius:2px;background:var(--surf);
...
.axiom-hero:hover::before,.axiom-hero:hover::after{width:44px;height:44px;}

/* scan line sweep */
.axiom-hero .scanline{
  position:absolute;left:0;right:0;height:2px;
  background:linear-gradient(90deg,transparent,rgba(65,244,224,0.25),transparent);
  animation:scanline 3.5s linear infinite;pointer-events:none;z-index:2;
}
...
.hero-eyebrow{
  ...
  animation:slideLeft 0.6s var(--ease) 0.1s both;
}
...
.hero-title span{color:var(--cyan)!important;animation:txt-flicker 6s ease infinite;}
.hero-sub{
  font-family:var(--fm)!important;font-size:11px!important;letter-spacing:1.5px!important;
...
.hero-tag{
  ...
  font-family:var(--fm)!important;font-size:10px!important;letter-spacing:2px!important;
  color:var(--amber)!important;text-transform:uppercase!important;
  animation:fadeIn 1s var(--ease) 0.5s both;
  transition:border-color 0.2s,background 0.2s;
}
.hero-tag:hover{border-color:rgba(245,166,35,0.6);background:rgba(245,166,35,0.1);}
...
.kpi-tile{
  ...
  animation:kpi-pop 0.5s var(--ease) both;
}
.kpi-tile:hover{border-color:rgba(65,244,224,0.28);transform:translateY(-2px);}
.kpi-tile::after{
...
.kpi-tile::before{
  ...
  background:linear-gradient(90deg,transparent,rgba(65,244,224,0.05),transparent);
  transition:left 0.5s ease;
}
.kpi-tile:hover::before{left:150%;}
...
.kpi-val{font-family:var(--fh)!important;font-size:2rem!important;font-weight:800!important;line-height:1!important;color:#fff!important;letter-spacing:-1px!important;}
...
.panel-lbl{
  font-family:var(--fm)!important;font-size:9px!important;letter-spacing:3px!important;
  text-transform:uppercase!important;color:var(--cyan)!important;margin-bottom:20px;
  display:flex;align-items:center;gap:10px;
}
.panel-lbl::after{content:'';flex:1;height:1px;background:linear-gradient(90deg,var(--bdr2),transparent);}

/* ══════════════════════════════════════════════
   LOADING OVERLAY (full-screen analysis animation)
   ══════════════════════════════════════════════ */
.axiom-loader{
...
.loader-hex svg{animation:hex-spin 2s linear infinite;}
.loader-ring{
  width:120px;height:120px;border-radius:50%;
  border:1px solid rgba(65,244,224,0.15);
...
.loader-title{
  font-family:var(--fh)!important;font-size:1.1rem!important;font-weight:700!important;
  letter-spacing:4px!important;color:var(--cyan)!important;text-transform:uppercase!important;
  margin-bottom:12px;
}
.loader-step{
  font-family:var(--fm)!important;font-size:10px!important;letter-spacing:2.5px!important;
  color:rgba(65,244,224,0.6)!important;text-transform:uppercase!important;
  min-height:18px;
}
.loader-bar-wrap{
  width:280px;height:2px;background:rgba(255,255,255,0.06);
...
.gauge-wrap{
  ...
  animation:scaleIn 0.6s var(--ease) 0.2s both;
}
...
.p-ok  {background:rgba(46,204,113,0.06);border:1px solid rgba(46,204,113,0.3);color:#2ecc71!important;animation:fadeUp 0.5s var(--ease) both, success-ripple 1s ease 0.5s;}
.p-fail{background:rgba(255,71,87,0.08); border:1px solid rgba(255,71,87,0.4); color:#ff4757!important;animation:fadeUp 0.5s var(--ease) both, alarm-flash 1.2s ease infinite;}
...
.s-row:hover{background:rgba(65,244,224,0.04);border-color:rgba(65,244,224,0.18);}
...
/* ══════════════════════════════════════════════
   FEATURE BARS
   ══════════════════════════════════════════════ */
.f-item{margin-bottom:15px;animation:slideRight 0.4s var(--ease) both;}
...
.f-fill{height:100%;border-radius:99px;background:linear-gradient(90deg,var(--cyan),rgba(65,244,224,0.5));transition:width 1.2s var(--ease);}

/* ══════════════════════════════════════════════
   ACTION BLOCKS
   ══════════════════════════════════════════════ */
.a-block{
  display:flex;gap:16px;padding:14px 0;border-bottom:1px solid rgba(255,255,255,0.05);
  animation:fadeUp 0.4s var(--ease) both;
  transition:background 0.15s;
}
.a-block:hover{background:rgba(255,255,255,0.015);}
.a-block:last-child{border-bottom:none;}
.a-icon{width:36px;height:36px;flex-shrink:0;display:flex;align-items:center;justify-content:center;border-radius:2px;font-size:15px;transition:transform 0.2s;}
.a-block:hover .a-icon{transform:scale(1.1);}
...
.h-row{color:var(--muted)!important;transition:background 0.1s;}
.h-row:hover{background:rgba(65,244,224,0.03);}
...
.stButton>button{
  ...
  padding:14px!important;transition:all 0.2s!important;position:relative!important;overflow:hidden!important;
}
.stButton>button::after{
  content:'';position:absolute;inset:0;
  background:linear-gradient(90deg,transparent,rgba(65,244,224,0.08),transparent);
  transform:translateX(-100%);transition:transform 0.4s ease;
}
.stButton>button:hover{
  background:rgba(65,244,224,0.06)!important;
  box-shadow:0 0 30px rgba(65,244,224,0.18)!important;
...
.sb-logo span{color:var(--cyan)!important;animation:txt-flicker 8s ease infinite;}
...
.sound-toggle{
  ...
  padding:6px 14px;border:1px solid rgba(65,244,224,0.3);border-radius:2px;
  background:rgba(65,244,224,0.04);cursor:pointer;
  font-family:var(--fm)!important;font-size:9px!important;letter-spacing:2px!important;
  color:var(--cyan)!important;text-transform:uppercase!important;
  transition:all 0.2s;user-select:none;
}
.sound-toggle:hover{background:rgba(65,244,224,0.1);border-color:var(--cyan);}
.sound-toggle.muted{border-color:rgba(255,255,255,0.1);color:var(--dim)!important;background:transparent;}
</style>
"""

# ══════════════════════════════════════════════
#  INTERACTIVE JS (animations + sound triggers)
# ══════════════════════════════════════════════
INTERACTION_JS = """
<script>
(function(){
  if(window._axiomInteractReady) return;
  window._axiomInteractReady = true;

  // ── Boot sound on load ──
  document.addEventListener('click', function bootOnce(){
    window.playAxiom && window.playAxiom('boot');
  ...
  document.addEventListener('click', function(e){
    ...
    if(btn && !btn.closest('[data-testid="stSidebarCollapseButton"]') &&
              !btn.closest('[data-testid="collapsedControl"]')){
      window.playAxiom && window.playAxiom('click');
    }
  });
  ...
  document.addEventListener('input', function(e){
    if(e.target.type==='range'){
      window.playAxiom && window.playAxiom('ticker');
    }
  });
  ...
  function animateCounter(el, target, duration=800){
    ...
    function update(now){
      ...
      el.textContent = Number.isInteger(target) ? Math.round(val) : val.toFixed(1)+(target.toString().includes('%')?'%':'');
  ...
  function staggerPanels(){
    const panels = document.querySelectorAll('.panel, .kpi-tile');
    panels.forEach((p,i)=>{
  ...
  function addScanLines(){
    document.querySelectorAll('.panel').forEach(p=>{
      if(p.querySelector('.panel-scan')) return;
      const sl = document.createElement('div');
      sl.className='panel-scan';
      sl.style.cssText='position:absolute;left:0;right:0;height:1px;background:linear-gradient(90deg,transparent,rgba(65,244,224,0.15),transparent);animation:scanline '+(3+Math.random()*2).toFixed(1)+'s linear infinite;pointer-events:none;z-index:2;top:0;';
      p.style.position='relative';
      p.appendChild(sl);
    });
  }

  // ── Typing effect for panel labels ──
  function typeLabels(){
    document.querySelectorAll('.panel-lbl').forEach(el=>{
      const txt = el.childNodes[0];
      if(!txt || txt._typed) return;
      txt._typed = true;
  ...
  // Run on DOM ready and after Streamlit re-renders
  function init(){
    staggerPanels();
    setTimeout(addScanLines, 300);
    setTimeout(typeLabels,   200);
  }

  // Observe DOM changes (Streamlit rerenders)
...
# ══════════════════════════════════════════════
#  RESULT SOUND JS  (triggered after prediction)
# ══════════════════════════════════════════════
def result_sound_js(prediction: int) -> str:
    sound = "failure" if prediction == 1 else "success"
    return f"""
    <script>
    (function(){{
      // slight delay so page has rendered
      setTimeout(()=>{{ window.playAxiom && window.playAxiom('{sound}'); }}, 300);
      // animate gauge arc
      setTimeout(()=>{{
        const arcs = document.querySelectorAll('circle[stroke-dasharray]');
        arcs.forEach(a=>{{
          const orig = a.getAttribute('stroke-dasharray');
          a.setAttribute('stroke-dasharray','0 999');
          setTimeout(()=>a.setAttribute('stroke-dasharray', orig), 50);
        }});
      }}, 100);
    }})();
    </script>"""

def scan_sound_js() -> str:
    return "<script>setTimeout(()=>{ window.playAxiom && window.playAxiom('scan'); }, 100);</script>"

def warning_sound_js() -> str:
    return "<script>setTimeout(()=>{ window.playAxiom && window.playAxiom('warning'); }, 100);</script>"
...
def gauge_svg(prob:float)->str:
    pct=prob*100; r,cx,cy=68,88,88
    circ=2*math.pi*r; dash=circ*prob; gap=circ-dash
    ...
    <div class="gauge-wrap">
      <svg width="176" height="176" viewBox="0 0 176 176">
        ...
        <circle id="gauge-arc" cx="{cx}" cy="{cy}" r="{r}" fill="none"
    ...
    setTimeout(()=>{{
      ...
      if(arc) arc.setAttribute('stroke-dasharray','{dash:.2f} {gap:.2f}');
    }},100);
    </script>"""
...
def feature_bars_html(imp:dict)->str:
    ...
    for i,(name,val) in enumerate(sorted(imp.items(),key=lambda x:-x[1])):
        ...
        delay=i*0.08
        html+=f"""
        <div class="f-item" style="animation-delay:{delay:.2f}s;">
          <div class="f-meta"><span>{name}</span><span style="color:#41f4e0;">{val:.4f}</span></div>
          <div class="f-track"><div class="f-fill" style="width:0%;transition:width 1.2s cubic-bezier(0.16,1,0.3,1) {delay:.2f}s;"
               data-target="{pct:.1f}"></div></div>
        </div>"""
    html+="""
    <script>
    setTimeout(()=>{
      document.querySelectorAll('.f-fill[data-target]').forEach(el=>{
...
def actions_html(sv:dict)->str:
    ...
    if tw>200:   items.append(("crit","🔧","Replace Cutting Tool",f"Wear at {tw} min exceeds 200 min threshold. Immediate swap required."))
    elif tw>160: items.append(("warn","🔧","Schedule Tool Replacement",f"Wear at {tw} min — 80% of service life consumed. Plan swap within next shift."))
    else:        items.append(("ok","🔧","Tool Condition Nominal",f"Wear at {tw} min — well within 200 min operating limit."))
    rpm_v=sv.get("RPM",0)
    if rpm_v>2500:   items.append(("crit","⚡","Overspeed Condition",f"RPM at {rpm_v} exceeds 2 500 limit. Reduce drive frequency immediately."))
    elif rpm_v>2200: items.append(("warn","⚡","Speed Approaching Limit",f"RPM at {rpm_v} — within 12% of ceiling. Monitor thermal rise."))
    else:            items.append(("ok","⚡","Rotational Speed Normal",f"RPM at {rpm_v} — within nominal operating band."))
    tq=sv.get("Torque (Nm)",0)
    if tq>65:   items.append(("crit","🌀","Torque Overload",f"Torque at {tq} Nm exceeds 65 Nm rating. Inspect gearbox."))
    elif tq>55: items.append(("warn","🌀","Elevated Drive Torque",f"Torque at {tq} Nm — check feed rate and workpiece hardness."))
    ...
    if dT>12:   items.append(("crit","🌡","Critical Thermal Delta",f"ΔT = {dT:.1f} K — cooling circuit degraded. Halt and inspect."))
    elif dT>9:  items.append(("warn","🌡","Elevated Thermal Delta",f"ΔT = {dT:.1f} K — monitor coolant flow rate."))
    else:       items.append(("ok","🌡","Thermal Balance Normal",f"ΔT = {dT:.1f} K — heat transfer within design limits."))
    html=""
...
def history_html(history:list)->str:
    ...
    for e in reversed(history[-8:]):
        ...
        stxt="FAILURE" if e["prediction"]==1 else "NOMINAL"
        h+=f'<div class="h-row"><span>{e["time"]}</span><span>TYPE {e["machine_type"]}</span><span style="color:#41f4e0;">{e["probability"]:.1%}</span><span style="color:{st_};font-weight:600;">{stxt}</span></div>'
    return h


# ══════════════════════════════════════════════
#  INJECT SOUND + CSS (once per session)
# ══════════════════════════════════════════════
...
with st.sidebar:
    ...
    if st.button(sound_label, key="sound_btn"):
    ...
    for sname,val in sv.items():
        ...
        if st_ in ("warn","crit"): any_warn=True
    ...
    # play warning sound if sensor out of range
    if any_warn and st.session_state.sound_on:
        st.markdown(warning_sound_js(), unsafe_allow_html=True)
...
<div class="axiom-hero">
  ...
  <svg style="position:absolute;right:48px;top:50%;transform:translateY(-50%);width:180px;height:140px;opacity:0.07;"
    ...
    <circle cx="50" cy="27" r="3" fill="white"><animate attributeName="opacity" values="0.2;1;0.2" dur="2s" repeatCount="indefinite"/></circle>
    <circle cx="90" cy="61" r="3" fill="white"><animate attributeName="opacity" values="0.2;1;0.2" dur="2.5s" repeatCount="indefinite"/></circle>
    <circle cx="130" cy="95" r="3" fill="white"><animate attributeName="opacity" values="0.2;1;0.2" dur="1.8s" repeatCount="indefinite"/></circle>
  </svg>
</div>
""", unsafe_allow_html=True)
...
if run_btn:
    ...
    if st.session_state.sound_on:
        st.markdown(scan_sound_js(), unsafe_allow_html=True)
    ...
    # result sound
    if st.session_state.sound_on:
        st.markdown(result_sound_js(pred), unsafe_allow_html=True)
...
with col_c1:
    ...
    ax.xaxis.grid(True,color=M_GRID,lw=0.5,zorder=0); ax.set_axisbelow(True); ax.tick_params(length=0)
...
with col_c2:
    ...
    if model and hasattr(model,"feature_importances_"):
        ...
        except: imp=dict(zip(FT_NAMES,[0.18,0.15,0.22,0.20,0.14,0.06,0.05]))
    else: imp=dict(zip(FT_NAMES,[0.18,0.15,0.22,0.20,0.14,0.06,0.05]))
...
if len(st.session_state.history)>1:
    ...
    with col_t:
        ...
        ax2.set_ylim(-0.02,1.05); ax2.set_xlim(0.5,max(xs)+0.5)
        ...
        ax2.yaxis.grid(True,color=M_GRID,lw=0.5,zorder=0); ax2.set_axisbelow(True); ax2.tick_params(length=0)
        plt.tight_layout(pad=0.3)
